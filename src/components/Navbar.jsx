        ...
        <>
            <style>
                ...
                .nav-link {
                    ...
                    color: inherit;
                    cursor: pointer;
                    transition: color 0.2s, transform 0.3s ease-in-out; /* Add transform for smooth rotation */
                    white-space: nowrap;
                }
                .nav-link:hover {
                    color: #ffd700;
                ...
                .left-link-1 {
                    margin-right: 7rem; /* Adjust horizontal spacing */
                    margin-top: 0.5rem;
                ...
                .right-link-1 {
                    margin-left: 7rem; /* Adjust horizontal spacing */
                    transform: rotate(0deg);
                }
                .right-link-2 {
                    margin-left: 7rem; /* Adjust horizontal spacing */
                ...
                .scrolled .left-link-1 {
                    transform: rotate(0deg); /* Less rotation when scrolled */
                    margin-top: 0;
                }
                .scrolled .left-link-2 {
                    transform: rotate(0deg); /* Less rotation when scrolled */
                    margin-top: 0;
                }
                .scrolled .right-link-1 {
                    transform: rotate(0deg); /* Less rotation when scrolled */
                    margin-top: 0;
                }
                .scrolled .right-link-2 {
                    transform: rotate(0deg); /* Less rotation when scrolled */
                    margin-top: 0;
                }
                `}
            ...
            {/* Navlinks using standard HTML/CSS with individual styling */}
            <div className={`nav-links ${isScrolled ? 'scrolled' : ''}`}>
                <a onClick={() => navigate('/')} className="nav-link left-link-1">Home</a>
                {currentUser && (
                    <a onClick={() => navigate('/profile')} className="nav-link left-link-2">Profile</a>
                )}
                {/* Spacing for the logo */}
                <div style={{ width: isScrolled ? '150px' : '200px', transition: 'width 0.3s ease-in-out' }}></div>
                <a onClick={currentUser ? handleLogout : () => navigate('/auth')} className="nav-link right-link-1">
                    {currentUser ? 'Logout' : 'Login'}
                </a>
                {/* This link is for a different purpose if needed, e.g. 'Contact' */}
                <a className="nav-link right-link-2">Settings</a>
            </div>

            {/* Independent logo container (unchanged) */}
